...
enum CreatePDFState {NOTHING, SUCCESS, ERROR};

export default function DownloadReport(props: DownloadReportProps): JSX.Element {
  const [loading, setLoading] = useState<boolean>(false);
  const [savedFilePath, setSavedFilePath] = useState<CreatePDFState>(CreatePDFState.NOTHING);
  ...
  const createPDF = async (html: string) => {
    ...
    await StorageAccessFramework.createFileAsync(permissions.directoryUri, "Report", 'application/pdf')
      .then(async (uri) => {
  ...
  return (
    <>
      <Button icon={Download} scaleIcon={1.2} marginHorizontal="$2" theme="active" onPress={handleGetPDF} disabled={loading} opacity={loading ? buttonDisabledOpacitiy : undefined}>Get PDF report</Button>
      {loading &&
